"""Build a Whoosh index from cleaned articles using Whoosh's default analyzer."""
...
try:
    from src.schema import get_whoosh_default_schema
except ModuleNotFoundError:
    from schema import get_whoosh_default_schema


PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_INPUT_DB_PATH = PROJECT_ROOT / "data/processed/wiki_articles_step1_clean.sqlite3"
DEFAULT_INDEX_DIR = PROJECT_ROOT / "index/whoosh_default_index"
...
def create_index(index_dir: Path = DEFAULT_INDEX_DIR):
    ...
    return whoosh_index.create_in(index_dir, get_whoosh_default_schema())
...
def materialize_whoosh_default_index(
    ...
    for article in iter_articles_from_db(input_db_path):
        ...
        if total % 1000 == 0:
            ...
            print(f"[processor4_whoosh_index] Articles: {total} | Elapsed: {elapsed:.2f}s")
    ...
    elapsed = time.time() - start_time
    print(f"[processor4_whoosh_index] Finished | Articles: {total} | Elapsed: {elapsed:.2f}s")
    return total


if __name__ == "__main__":
    total = materialize_whoosh_default_index()
