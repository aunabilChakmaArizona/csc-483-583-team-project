"""Build a Cole-style Whoosh index using only the first sentence of each article."""
...
DEFAULT_INDEX_DIR = PROJECT_ROOT / "index/whoosh_cole_first_sentence_index"


def materialize_whoosh_cole_first_sentence_index(
    input_db_path: Path = DEFAULT_INPUT_DB_PATH,
    index_dir: Path = DEFAULT_INDEX_DIR,
    batch_size: int = 1000,
) -> int:
    return materialize_lead_index(
        lead_mode="first_sentence",
...
if __name__ == "__main__":
    total = materialize_whoosh_cole_first_sentence_index()
