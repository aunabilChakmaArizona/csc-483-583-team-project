"""Build a Cole-style Whoosh index using the first two sentences of each article."""
...
DEFAULT_INDEX_DIR = PROJECT_ROOT / "index/whoosh_cole_first_two_sentences_index"


def materialize_whoosh_cole_first_two_sentences_index(
    input_db_path: Path = DEFAULT_INPUT_DB_PATH,
    index_dir: Path = DEFAULT_INDEX_DIR,
    batch_size: int = 1000,
) -> int:
    return materialize_lead_index(
        lead_mode="first_two_sentences",
...
if __name__ == "__main__":
    total = materialize_whoosh_cole_first_two_sentences_index()
