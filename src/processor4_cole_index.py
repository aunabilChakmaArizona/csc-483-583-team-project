"""Build a Whoosh index that mirrors Cole's processor-4 implementation."""
...
DEFAULT_INDEX_DIR = PROJECT_ROOT / "index/whoosh_cole_index"
...
def materialize_whoosh_cole_index(
    ...
    for article in iter_articles_from_db(input_db_path):
        ...
        if total % batch_size == 0:
            ...
            print(f"[processor4_cole_index] Indexed: {total} | Elapsed: {elapsed:.2f}s")

    print("[processor4_cole_index] Committing index to disk (this may take a moment)...")
    writer.commit()

    elapsed = time.time() - start_time
    print(
        f"[processor4_cole_index] Done | Indexed: {total} | "
...
if __name__ == "__main__":
    total = materialize_whoosh_cole_index()
